...
def generate_ai_scenario(incident_type, difficulty):
    # 🚨 FIXED: Updated prompt to force the AI to write real telemetry!
    prompt = f"""
        ...
        Format:
        {{
          "scenario_title": "Catchy title for the incident",
          "scenario_brief": "A high-level narrative summary of the situation.",
          "injects": [
            {{"stage":"Detect","text":"RAW TECHNICAL TELEMETRY ONLY. Example: IDS Alert, server log snippet, or firewall flag. Do NOT repeat the narrative here."}},
            {{"stage":"Analyse","text":"Update on the investigation."}},
            {{"stage":"Remediation","text":"Update on containment efforts."}}
          ]
        }}
    """.strip()
